...
fn map_with_ref_recursively_projects() {
    // Regression test: conversation-bundle.json uses `"type": "map"` with a
    // separate `"ref"` attribute for nested types (e.g., HandoffInfo inside
    // ConversationItem).  The projection engine must treat this the same as
    // `"type": "ref"` and recursively decode nested fields to named keys.
    let dir = tempdir().expect("tempdir");
    let mut registry = Registry::open(dir.path()).expect("open registry");
...
fn array_shorthand_ref_recursively_projects() {
    // Regression test: conversation-bundle.json uses `"items": { "ref": "T" }`
